 ...
 */
class German_LocalePackCa_Block_System_Config_Form_Fieldset_Instructions
    extends Mage_Adminhtml_Block_System_Config_Form_Fieldset
{
    public function render(Varien_Data_Form_Element_Abstract $element)
    {
    	$nodepath = "modules/German_LocalePackCa";
        $helper = Mage::helper("localepackca");
	...
	if (!array_key_exists("German_LocaleFallback", $modules)) {
        $html .= "<p>".$helper->__("Note: Install the extension %s, so you can use %s as a <a href='%s'>fallback language</a>.",
        		...
        		"fr_FR",
        		$section_link )."</p>";
	} else if (!array_key_exists("German_LocalePackFr", $modules)) {
        $html .= "<p>".$helper->__("Note: Install the language package %s to use it as a <a href='%s'>fallback language</a>.",
        		"<a href='https://github.com/MaWoScha/German_LocalePack_fr_FR'>German LocalePackFr</a>",
        		$section_link )."</p>";
	}
        $html .= "<p style='margin-top:20pt;'>";
		$html .= $helper->__("On the <a href='%s'>Magento Connect page of the French-Canadian language pack</a>, you can find more information on the latest versions.",
